...
<div class="space-y-4">
    @forelse($kegiatan as $item)
        ...
        <{{ $isSelesai ? 'div' : 'a' }} 
            @if(!$isSelesai) href="{{ route('peserta.kegiatan.show', $item) }}" @endif
            class="block p-5 rounded-2xl bg-white border border-slate-200/50 shadow-sm {{ $isSelesai ? 'opacity-75 cursor-not-allowed' : 'hover:shadow-md hover:border-emerald-200 transition-all' }}">
            <div class="flex items-start justify-between gap-4">
                <div class="flex-1">
                    ...
                    <div class="flex flex-wrap gap-4 mt-2 text-xs text-slate-500">
                        <span><i class="fas fa-calendar mr-1 {{ $isSelesai ? 'text-slate-300' : 'text-emerald-500' }}"></i>{{ $item->waktu_mulai->translatedFormat('d M Y, H:i') }}</span>
                        <span><i class="fas fa-location-dot mr-1 {{ $isSelesai ? 'text-slate-300' : 'text-emerald-500' }}"></i>{{ $item->tempat }}</span>
                        <span><i class="fas fa-file-lines mr-1 {{ $isSelesai ? 'text-slate-300' : 'text-emerald-500' }}"></i>{{ $item->materi->count() }} materi</span>
                    </div>
                </div>
                @if(!$isSelesai)
