...
<div class="space-y-4">
    ...
    <a href="{{ route('narasumber.kegiatan.show', $item) }}" class="block p-5 rounded-2xl bg-white border border-slate-200/50 shadow-sm hover:shadow-md hover:border-amber-200 transition-all">
        <div class="flex items-start justify-between gap-4">
            <div class="flex-1">
                ...
                <div class="flex flex-wrap gap-4 mt-2 text-xs text-slate-500">
                    <span><i class="fas fa-calendar mr-1 text-amber-500"></i>{{ $item->waktu_mulai->translatedFormat('d M Y, H:i') }}</span>
                    <span><i class="fas fa-location-dot mr-1 text-amber-500"></i>{{ $item->tempat }}</span>
                    <span><i class="fas fa-users mr-1 text-amber-500"></i>{{ $item->peserta->count() }} peserta</span>
                </div>
            </div>
