    ...
    <form method="POST" action="{{ route('admin.kegiatan.store') }}" class="space-y-6">
        ...
        <div class="p-6 rounded-2xl bg-white border border-slate-200/50 shadow-sm space-y-5">
            ...
            <div>
                ...
                <textarea name="alamat_lengkap" rows="2" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 text-sm resize-none" placeholder="Alamat lengkap lokasi kegiatan">{{ old('alamat_lengkap') }}</textarea>
            </div>

            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Latitude</label>
                    <input type="number" step="any" name="latitude" value="{{ old('latitude') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 text-sm" placeholder="-6.9175">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Longitude</label>
                    <input type="number" step="any" name="longitude" value="{{ old('longitude') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-400/50 text-sm" placeholder="107.6191">
                </div>
            </div>
        </div>
